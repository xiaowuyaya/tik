...
class LcuService extends Service {
  ...
  async getGameDetailByGameId (gameId) {
    ...
    for (let i = 0; i < data.participantIdentities.length; i++) {
      let player = {}
      player.championAvatar = this.getAvatarUrlByChampId(
      ...
      player.totalDamageDealtToChampions =
        data.participants[i].stats.totalDamageDealtToChampions
  ...
  sourcingRules (historyMatches) {
    ...
    let SOURCE =
      (KDA * 10 * 0.4 + WIN_RATE * 100 * 0.4 + killingSpreesSource * 0.2) * 10
  ...
  async dataAnalysis (formatedPlayerInfoList) {
    ...
    for (let i = 0; i < formatedPlayerInfoList.length; i++) {
      ...
      if (!formatedPlayerInfoList[i].isBot) {
        const puuid = await api.getPuuidBySummonerName(
          formatedPlayerInfoList[i].summonerName
        )
    ...
    for (let i = 0; i < blackList.length; i++) {
      ...
      for (let j = 0; j < formatedList.length; j++) {
        ...
        if (player.summonerName == ban.blackName) {
          // 发送信息到聊天框
          await this.snedMsgInChampSelect(
            blackNoticeToAll ? "all" : "me",
            `[对局助手]：玩家 ${ban.blackName} 在你的黑名单中, 原因：${ban.reason}`
          )
        }
      }
    }
