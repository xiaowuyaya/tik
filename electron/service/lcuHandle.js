...
const { blacklistConfig, appConfig } = require('../store/index')
const Service = require('ee-core').Service
const c = require('../utils/cache')

class LcuHandleService extends Service {
  constructor(ctx) {
    super(ctx)
  }

  async currentSummonerInfo() {
    let currentSummoner = {
      gameStatus: '',
      avatar: '',
      summonerLevel: '',
    ...
    currentSummoner.rank.rankedHighest = `${utils.translate('rank', rankStatus.highestPreviousSeasonEndTier)} ${
      rankStatus.highestPreviousSeasonEndDivision
    }`
    currentSummoner.rank.rankedSoloTier = rankStatus.queueMap.RANKED_SOLO_5x5.tier
    currentSummoner.rank.rankedSolo = `${utils.translate('rank', rankStatus.queueMap.RANKED_SOLO_5x5.tier)} ${
      rankStatus.queueMap.RANKED_SOLO_5x5.division
    ...
    currentSummoner.rank.rankedFlex = `${utils.translate('rank', rankStatus.queueMap.RANKED_FLEX_SR.tier)} ${
      rankStatus.queueMap.RANKED_FLEX_SR.division
    ...
    let rencentlyPositionTemp = {}
    let rencentlyPosition = {}
  ...
  async checkSummonerExist(summonerName) {
    const summoner = await api.getSummonerInfoBySummonerName(summonerName)
    return !(summoner.errorCode && summoner.errorCode === 'RPC_ERROR')
  }

  async searchSummonerInfo(summonerName) {
    let currentSummoner = {
      avatar: '',
      summonerLevel: '',
      summonerId: '',
  ...
  async historyMatchesList(summonerName, page, limit = 7) {
    const summoner = await api.getSummonerInfoBySummonerName(summonerName)
  ...
  async getPanelData(status) {
    let data = c.get('panelData')
    if (!data) data = { orderList: [], chaosList: [] }
    try {
      const sendTogetherGame =  appConfig.get('sendTogetherGame')
      if (status == 'ChampSelect' && data.orderList.length < 5) {
        data = await this.getPanelDataInChampSelect()
        c.put('panelData', data)
        ...
        if (data.orderTogether.arr1.length > 1 && sendTogetherGame) {
          ...
          await api.sendMsgInChampSelect('all', `[tik对局助手] ${msg} 为组队玩家`)
        }
      ...
      if (status == 'InProgress' && data.chaosList.length == 0) {
        data = await this.getPanelDataInProgress()
        c.put('panelData', data)
      }
    } catch (err) {}
    return JSON.stringify(c.get('panelData'))
  }
  ...
  async getPanelDataInChampSelect() {
    ...
    for (let i = 0; i < sessionRes.myTeam.length; i++) {
      ...
      if (sessionRes.myTeam[i].summonerId != 0) {
        ...
        const matchesData = await this.sourcingRules(hisotryMatches)
        temp.matches = matchesData

        const rank = await api.getRankedStatusInfoByPuuid(summonerInfo.puuid)
        temp.rank = {
          rankedSolo: `${utils.translate('rank', rank.queueMap.RANKED_SOLO_5x5.tier)} ${
            rank.queueMap.RANKED_SOLO_5x5.division
          }`,
          rankedFlex: `${utils.translate('rank', rank.queueMap.RANKED_FLEX_SR.tier)} ${
            rank.queueMap.RANKED_FLEX_SR.division
          }`,
        }
      } else {
        temp.summonerId = sessionRes.myTeam[i].summonerId
    ...
    const res = this.sortAndSetDesignation(playerList)

    // 获取黑名单列表
    const blackList = blacklistConfig.get('blacklist')
      ...
      for (let j = 0; j < res.length; j++) {
        ...
        if (player.summonerName == ban.banName) {
          await api.sendMsgInChampSelect(
            ...
            `[Tik 英雄联盟对局助手]：玩家 ${ban.banName} 在你的黑名单中, 原因：${ban.reason}`,
          )
          break
        }
      }
    ...
    const orderTogether = this.checkTogether(tempOrderArr)
  ...
  async getPanelDataInProgress() {
    ...
    panelData.orderList = this.sortAndSetDesignation(panelData.orderList)
    panelData.chaosList = this.sortAndSetDesignation(panelData.chaosList)
    ...
    panelData.orderTogether = this.checkTogether(tempOrderArr)
    panelData.chaosTogether = this.checkTogether(tempChaosArr)

    return panelData
  }

  async sourcingRules(hisotryMatches) {
  ...
  sortAndSetDesignation(list) {
    ...
    const summonerTitle = appConfig.get('typeTitle')
    // 设置排名
...
LcuHandleService.toString = () => '[class LcuHandleService]'
module.exports = LcuHandleService
