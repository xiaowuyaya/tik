const { appConfig, blacklistConfig } = require('../store/index')
...
exports.currentSummonerInfo = async () => {
  let currentSummoner = {
    gameStatus: '',
    avatar: '',
    summonerLevel: '',
  ...
  const historyMatch = await api.getHistoryMatchesByPuuid(summoner.puuid, 0, 40)
  currentSummoner.historyMatch = historyMatch.games.games.reverse()
  ...
  let totalDamage = 0
  let positions = []
  _.forEach(historyMatch.games.games, match => {
...
exports.checkSummonerExist = async summonerName => {
...
exports.searchSummonerInfo = async summonerName => {
  let currentSummoner = {
    avatar: '',
    summonerLevel: '',
    summonerId: '',
  ...
  const historyMatch = await api.getHistoryMatchesByPuuid(summoner.puuid, 0, 21)
  currentSummoner.historyMatch = historyMatch.games.games.reverse()
...
exports.historyMatchesList = async (summonerName, page, limit = 7) => {
  const summoner = await api.getSummonerInfoBySummonerName(summonerName)
...
exports.matchesDetail = async gameId => {
...
exports.confirmChampionById = async (championId, confirm) => {
...
exports.getPanelData = async status => {
  let data = panelDataConfig.get('panelData')

  try {
    if (status == 'ChampSelect' && data.orderList.length < 5) {
      data = await getPanelDataInChampSelect()
      panelDataConfig.set('panelData', data)
  ...
  return panelDataConfig.get('panelData')
}
...
const getPanelDataInProgress = async () => {
  ...
  for (let i = 0; i < playerList.length; i++) {
    if (!playerList[i].isBot) {
      ...
      res.profileIcon = this.getChampionAvatarByCnName(playerList[i].championName)
...
const sourcingRules = async historyMatches => {
...
const sortAndSetDesignation = list => {
  list.sort((a, b) => {
...
exports.changeTiger = async tiger => {
  try {
...
exports.changeStatus = async status => {
  try {
    let r = await api.getPlayerChatInfo()
...
exports.useRunePage = async data => {
  try {
    // 获取所有符文
