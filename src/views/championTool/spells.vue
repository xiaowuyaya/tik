<template>
  <div class="h-full w-full flex flex-col">
    ...
    <div class="mt-2">
      <!-- 无数据 -->
      <div v-if="spellsData.length == 0" class="text-base font-extrabold text-center text-red-500 ">暂无数据,当前游戏状态：{{
          gameStatus
      }}</div>
      <!-- 有数据 -->
...
onMounted(async () => {
  gameStatus.value = utils.translate('status', await handle.lcuApi.getGameStatus())
...
async function getSpellData(status: string) {
  ...
  if (status == 'InProgress' || status == '游戏中') {
    ...
    spellsData.value = await handle.getPlayerSpells()
