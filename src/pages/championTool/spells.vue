<template>
  <div class="h-full w-full flex flex-col">
    <div class="text-base text-center text-white user-drag">
      <span class="bg-sky-600 px-4">Eko 英雄联盟工具箱 手动技能提醒 (可拖动区)</span>
    </div>
    <div class="mt-2">
      <!-- 无数据 -->
...
onMounted(async () => {
  gameStatus.value = utils.translate('status', await ipcRenderer.invoke('controller.lcuHandle.getGameStatus', {}))
  await getSpellData(gameStatus.value)
})

// 持续监听玩家游戏状态变化,当状态满足条件时重新夹在数据
ipcRenderer.ipc.on('playerStatus', async (event, data) => {
...
async function getSpellData(status: string) {
  if (status == 'InProgress' || status == '游戏中') {
    spellsData.value = await ipcRenderer.invoke('controller.lcuHandle.getPlayerSpells', {})
...
async function spellTimeRecord(championName: string, summonerName: string, spellName: string, cooldownBurn: number) {
  ipcRenderer.invoke('controller.common.spellsWindowHandleSpellsTime', {
    championName,
    summonerName,
    spellName,
    cooldownBurn,
