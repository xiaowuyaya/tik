  ...
  <div class="h-full w-full">
    ...
    <div class="bg-gray-100 h-full p-2">
      <a-card :hoverable="true" :header-style="{ border: 'none' }" v-if="mode == 0">
        ...
        <div class="flex">
          <div class="flex items-center flex-1 justify-between bg-gray-100 p-2 mx-1 rounded"
            v-for="(item, index) in champion.data.summary.summary.positions" :key="index"
      ...
      <a-card class="mt-2" :hoverable="true" :header-style="{ border: 'none' }">
        ...
        <div class="flex justify-between">
          <div class="flex items-center justify-between bg-gray-100 p-2 rounded"
            v-for="(summoner_spells, index) in champion.data.summoner_spells.slice(0, 2)" :key="index">
            <div class="flex items-center justify-center flex-1">
              <img class="rounded-full w-[34px] h-[34px] last:ml-[-10px]" v-for="(item, index) in summoner_spells.ids"
                :key="index" :src="findDetailObj('spells', item).image_url" />
            </div>
        ...
        <div class="pb-2">
          ...
          <div class="flex flex-col">
            <div class="flex justify-center items-center px">
              <img class="w-[38px] h-[38px] rounded-full ml-2" :src="champion.data.summary.meta.passive.image_url"
                alt="" />
              <img class="w-[38px] h-[38px] rounded-full ml-2" :src="champion.data.summary.meta.spells[0].image_url"
                alt="" />
              <img class="w-[38px] h-[38px] rounded-full ml-2" :src="champion.data.summary.meta.spells[1].image_url"
                alt="" />
              <img class="w-[38px] h-[38px] rounded-full ml-2" :src="champion.data.summary.meta.spells[2].image_url"
                alt="" />
              <img class="w-[38px] h-[38px] rounded-full ml-2" :src="champion.data.summary.meta.spells[3].image_url"
                alt="" />
            </div>
    ...
    <a-drawer :drawer-style="{ background: 'rgba(243, 244, 246, 1)', padding: '-14px -10px !important' }"
      :header="false" :visible="drawerShow" height="80%" placement="bottom" :footer="false"
      ...
      <el-scrollbar height="100%">
        <div class="py-2 bg-light-50 rounded mb-2" v-for="(core_item, index) in champion.data.runes" :key="index">
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex flex-col">
              ...
              <div class="flex items-center">
                <img class="w-[34px] h-[34px] ml-2" v-for="(core_item_item, item_index) in core_item.primary_rune_ids"
                  :key="item_index" :src="findDetailObj('runes', core_item_item).image_url" />
              </div>
              <div class="flex justify-between mt-2">
                <div class="flex items-center">
                  <img class="w-[28px] h-[28px] ml-2"
                ...
                <div class="flex items-center">
                  <img class="w-[16px] h-[16px] ml-1" v-for="(core_item_item, item_index) in core_item.stat_mod_ids"
          ...
          <!-- footer -->
          <div class="flex items-center justify-between">
            <span class="px-2 text-sm text-blue-500">胜率 {{ ((core_item.win / core_item.play) * 100).toFixed(2)
            }}%</span>
...
import { positionIconByName } from '../../assets/rank-position'
...
const getChampionData = async (mode: number, championName: string, position?: string) => {
  ...
  const r = await $api.getChampionSkinListById(champion.value.data.summary.meta.id)
  backgroundImg.value = await $api.getLcuImgBase64(r.skins[0].splashPath)
 ...
 */
const hideWin = async () => {
  await ipcRenderer.invoke('rune-hide', {})
}
...
const useSelectRune = async (item: any) => {
  ...
  const data = {
    name: `${$tools.APP_TITLE}：${champion.value.data.summary.meta.name} `,
    order: item.id,
  ...
  // 发送信息
  await $api.sendMsgInChampSelect('ALL', `[Tik助手] 符文应用成功！`)
}
