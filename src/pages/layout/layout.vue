...
import { nextTick, onBeforeMount, onDeactivated, provide, ref } from 'vue-demi';
...
let keepLoginStatus = null

/* 更新信息通知 */
ipcRenderer.on('autoUpdater', (_event, data) => {
  Message[data.type]({
    content: data.msg,
    duration: 1500,
  })
  ...
  onBeforeMount(() => {
    ...
    // 定时任务保持当前账号的在线状态
    keepLoginStatus = setInterval(async () => {
  ...
  onDeactivated(() => {
    if (keepLoginStatus) clearInterval(keepLoginStatus)
  })
