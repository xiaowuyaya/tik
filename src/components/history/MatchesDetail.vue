      ...
      <div class="mt-4 border-t">
        ...
        >
          ...
          <div class="w-[78px] flex flex-col px-[1px] items-start justify-center ">
            <div :class="['text-base font-extrabold', item.participants[0].stats.win ? 'text-green-500' : 'text-red-500']">
    ...
    <a-card class="flex-1 ml-2" :hoverable="true" :header-style="{ border: 'none' }">
      ...
      <div>
        ...
        <a-row
          ...
          :class="[index == 4 ? 'border-gray-400' : '', item.summonerName == summonerName ? '!bg-blue-50 font-bold' : '']"
...
const getHistory = async () => {
  const recordData = await handle.historyMatchesList(summonerName.value, pagination.page, pagination.size);
...
const getHistoryDetail = async (gameId: string) => {
  ...
  matchesDetail.value = await handle.matchesDetail(gameId)
