<template>
  <div class="w-full h-full">
    <a-alert type="info" v-if="!champion.data"
      >数据加载中…… 如果服务器没有缓存数据，初次加载会花费较长时间（10秒左右），请耐心等待，后续将秒开</a-alert
    >
    <div class="w-full" v-if="champion.data">
      <c-scrollbar width="100%" height="710px" trigger="hover">
          ...
          <div class="flex justify-between">
            <div class="flex">
              <a-popover position="bottom" content="双击可在导入天赋窗口打开">
                <img
                  class="h-[82px] w-[82px] cursor-pointer"
                  @click="showChampionToolWindow"
                  :src="
                    champion.data.summary.meta.image_url +
                    '?image=c_crop,h_103,w_103,x_9,y_9/q_auto,f_webp,w_264&v=1651762875503'
                  "
                />
              </a-popover>
              <div class="ml-4 flex flex-col justify-end">
                ...
                <div class="flex">
                  <img
                    class="border-black mr-2 h-[30px] w-[30px]"
        ...
        <a-card class="mt-2">
          ...
          <a-row align="center">
            ...
            <a-col :span="4"
              >{{ ((champion.data.skills[0].win / champion.data.skills[0].play) * 100).toFixed(2) }}%
            </a-col>
          </a-row>
        </a-card>
        <!-- 出门装 -->
...
const showChampionToolWindow = async () => {
  ipcRenderer.send('rune-change-data', {
    mode: mode,
