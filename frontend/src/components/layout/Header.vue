<template>
  <header class="p-2 h-16 flex justify-between items-center user-drag border-b">
    <div class="flex items-center jusitify-between">
      <!-- LOGO -->
      <img
        class="w-10 no-drag cursor-pointer"
        src="@/assets/logo.png"
        @click="shell.openExternal('https://lol-tool.com')"
      />
      <!-- TITLE -->
    ...
    <div class="flex justify-between items-center no-drag">
      ...
      <a-button
        class="!p-1 !mr-1 !ml-4"
        type="text"
        @click="ipcRenderer.invoke('controller.common.mainWindowMinisize', {})"
      >
        <template #icon>
          <icon-minus />
        </template>
...
const handleQuitModal = () => {
  if (configStore.rememberQuit) {
    ipcRenderer.invoke('controller.common.mainWindowClose', {})
  } else {
