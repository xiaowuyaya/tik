  ...
  <div id="content" class="h-full w-full flex">
          ...
          <div class="f-full flex flex-col !ml-1">
            ...
            <div class="flex items-center justify-around">
              <div
                ...
                v-for="(obj, key, index) in (currentSummoner.rencentlyPosition as any)"
                :key="index"
              >
                <img class="w-12" :src="positionIcon[key]" />
                <div
                  :class="obj.wins / obj.total >= 0.5 ? '!bg-green-500' : '!bg-red-400'"
          ...
          <div class="flex h-full items-center justify-between">
            <div class="flex h-full flex-col w-full">
              ...
              <div class="mt-5 flex items-center justify-between w-full">
                <div
                  class="w-full flex flex-col justify-center items-center mx-2"
                  v-for="(item, index) in (currentSummoner.championMastery.masteries as any)"
                  :key="index"
                >
    ...
    <div class="!w-[324px] h-full ml-1">
      <a-card class="h-full" :hoverable="true">
        ...
        <block
          class="flex justify-between w-[188px] py-1.6 cursor-pointer"
          v-for="(item, index) in (currentSummoner.hisotryMatch.slice(0, 10) as any)"
          :key="index"
        >
...
onMounted(async () => {
  loading.value = true
  currentSummoner.value = await ipcRenderer.invoke('controller.lcuHandle.searchSummonerInfo', {summonerName})
  loading.value = false
})
