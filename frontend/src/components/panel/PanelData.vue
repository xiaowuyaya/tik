...
const getPanelData = async () => {
  ...
  // 获取当前玩家游戏状态
  gameStatus.value = await ipcRenderer.invoke('controller.lcuHandle.getGameStatus', {})
  getPlayerList(gameStatus.value)
  // 如果没有加载到数据，从缓存中获取
  if (!playerList.value.orderList.lenth || playerList.value.orderList.lenth == 0) {
    const panelData = await ipcRenderer.invoke('controller.common.getCache', { cache: 'panelData' })
...
const getPlayerList = async (status: string) => {
  if (status == 'InProgress' || status == 'ChampSelect') {
    playerList.value = JSON.parse(await ipcRenderer.invoke('controller.lcuHandle.getPanelData', { status }))
