<template>
  <div class="w-full h-full flex">
    <div class="flex flex-col w-[82%] h-full !mr-1">
        ...
        <a-card :hoverable="true" :header-style="{ border: 'none' }">
          ...
          <div class="pl-3">
            <a-row>
              ...
              <a-col :span="8">
                <a-form-item class="" label-col-flex="84px" label="发送组排信息">
                  <a-switch v-model="configStore.sendTogetherGame" @change="configStore.changeConfig">
                    <template #checked-icon>
                      <icon-check />
                    </template>
                    <template #unchecked-icon>
    ...
    <a-card class="flex flex-col flex-auto h-full !ml-1" :hoverable="true" :header-style="{ border: 'none' }">
      ...
      <a-row class="py-2" :gutter="12" align="center" justify="center">
        ...
        <a-col :span="8">
          <div class="truncate w-full cursor-pointer" @click="copyToClipboard(appPinaStore.logDir)">
            {{ appPinaStore.logDir }}
          </div>
        </a-col>
        <a-col :span="8">
          <a-button type="text" @click="shell.openPath(appPinaStore.logDir)">打开路径</a-button>
        </a-col>
      </a-row>
      <a-row class="py-2" :gutter="12" align="center" justify="center">
        <a-col :span="6" class="">配置文件</a-col>
        <a-col :span="8">
          <div class="truncate w-full cursor-pointer" @click="copyToClipboard(appPinaStore.configDir)">
            {{ appPinaStore.configDir }}
          </div>
        </a-col>
        <a-col :span="8">
          <a-button type="text" @click="shell.openPath(appPinaStore.configDir)">打开文件</a-button>
        </a-col>
      </a-row>
    </a-card>
  </div>
...
const handleUpdate = () => {
  ipcRenderer.invoke('controller.common.checkUpdate', {})
}
